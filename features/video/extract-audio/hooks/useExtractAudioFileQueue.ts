...
import { getFormatByExtension as getVideoFormatByExtension } from '@/features/video/shared/config/formats';
import { getVideoMetadata } from '@/features/video/shared/lib/videoUtils';
import { ExtractAudioFileItem } from './useExtractAudio';
import toast from 'react-hot-toast';

const MAX_FILES = 5;

/**
 * File queue for the Extract Audio tool
 */
export const useExtractAudioFileQueue = () => {
  const [files, setFiles] = useState<ExtractAudioFileItem[]>([]);
  ...
  const handleFiles = (incoming: File[]) => {
    ...
    for (const file of incoming) {
      ...
      if (!getVideoFormatByExtension(ext)) {
        if (!foundInvalid) {
    ...
    const newItems: ExtractAudioFileItem[] = accepted.map((file) => ({
  ...
  const updateFile = (id: string, patch: Partial<ExtractAudioFileItem>) => {
