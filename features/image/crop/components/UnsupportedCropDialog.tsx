...
interface UnsupportedCropDialogProps {
  open: boolean;
...
export const UnsupportedCropDialog = ({
  open,
  onOpenChange,
  fileName,
  fileExtension,
  onTryAnother,
}: UnsupportedCropDialogProps) => (
    ...
    <AlertDialogContent className="p-0 max-w-sm rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-2xl overflow-hidden">
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
              The crop tool can't process this file type
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="px-6 py-4 space-y-3">
        <div className="flex items-center gap-3 p-3 bg-red-50 dark:bg-red-950/20 border border-red-100 dark:border-red-900/40 rounded-xl">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[10px] text-red-500 dark:text-red-400 font-bold uppercase mt-0.5">
              .{fileExtension} — not supported for cropping
            </p>
          </div>
        </div>
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
          Convert your image to a supported format first, then use the crop
          tool.
        </p>
      </div>
      ...
      <div className="px-6 pb-6 flex flex-col gap-2">
        ...
        <button
          ...
          className="w-full py-2.5 text-sm font-semibold text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-all"
        >
