...
import { CompressFileItem as Item, formatBytes } from '../types/compress';

interface CompressFileItemProps {
  item: Item;
...
export const CompressFileItemRow = ({
  item,
  isProcessing,
  onDownload,
  onRemove,
}: CompressFileItemProps) => {
  const cfg = {
    idle: {
  ...
  return (
    <div
      className={`border rounded-xl p-3 flex items-center gap-3 transition-all duration-300 ${cfg.border} ${cfg.bg}`}
    >
      {/* Thumbnail */}
      ...
      <div className="shrink-0 flex items-center gap-1.5">
        ...
        {item.status === 'success' && (
          <button
            ...
            className="flex items-center gap-1.5 text-[10px] font-bold bg-emerald-600 hover:bg-emerald-700 active:scale-95 text-white px-2.5 py-1.5 rounded-lg transition-all"
          >
        ...
        <button
          ...
          className="p-1.5 text-slate-300 dark:text-slate-600 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-950/20 rounded-lg disabled:opacity-30 transition-all"
        >
